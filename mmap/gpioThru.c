...
int main(int argc, char *argv[]) {
    ...
    reg = *gpio_oe_addr;
    printf("GPIO12 configuration: %X\n", reg);
    reg &= ~GPIO_12;       // Set USR3 bit to 0
    *gpio_oe_addr = reg;
    printf("GPIO12 configuration: %X\n", reg);

    printf("Start copying GPIO_13 to GPIO_12\n");
    while(keepgoing) {
    	if(*gpio_datain & GPIO_13) {
            *gpio_setdataout_addr= GPIO_12;
    	} else {
            *gpio_cleardataout_addr = GPIO_12;
    	}
    }
